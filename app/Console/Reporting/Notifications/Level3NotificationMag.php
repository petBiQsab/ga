...
class Level3NotificationMag extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:level3notifyMag';
     ...
     * @var string
     */
    protected $description = 'Admin notification - 1 day after end of month';
            ...
            if (count($results) > 0) {
                ...
                if ($boss!=null)
                {
                    ...
                    $data->boss=$boss->name;
                     ...
                     Mail::to($user_email)->cc($bossOfManager)->send(new \App\Mail\Notifications\MagMinusday($data));

                }
                else
                {
                     Mail::to($user_email)->send(new \App\Mail\Notifications\MagMinusday($data));
                    //Mail::to('matus.baxant@bratislava.sk')->send(new \App\Mail\Notifications\MagMinusday($data));
                }
            }
        }
    }
